...
将小游戏分包后发布到微信平台时同样需要了许多问题。加载分包时，微信web开发工具报错net::ERR_BLOCKED_BY_CLIENT。我们发现这是因为加载分包与加载场景中的图片位于同一个生命周期回调内，导致某些图片在加载完成之前被请求。经过一番探索，并没有在网上找到解决这类问题的方案。最终我们选择在所有场景之前，加入一个preload场景，作为初始场景。该场景负责加载分包，并在加载完成之后切换到开始游戏的场景。
